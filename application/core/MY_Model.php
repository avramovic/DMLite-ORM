...
class MY_Model extends CI_Model implements Iterator
{
    ...
    public function __get($name)
    {
        ...
        if (in_array($singular, $this->has_many) || in_array($singular, $this->has_one)) {
            if (isset($this->_initRelated[$singular])) {
                return $this->_initRelated[$singular];
            }

            trigger_error("Try to include_related $name in your query");
    ...
    {
        //related model query
        $singular = self::_singular($method);
        if (in_array($singular, $this->has_many) || in_array($singular, $this->has_one)) {
    ...
    public function include_related($model, $initialize = false, $prefix = false)
    {
        if ($initialize && (strpos('/', $model) !== false)) {
            //cant initialize deep relationships
            trigger_error("Can't initialize deep relationship: {$model}");
        }
        ...
        $this->_include[] = array('model' => strtolower($model), 'prefix' => $prefix, 'initialize' => $initialize);
        return $this;
    }
    ...
    public function get()
    {
        ...
        //go through all included models avoiding those who has 'initialize' set to false

        if (!empty($this->_include)) {
            foreach ($this->_include as $include) {
                if ($include['initialize'] == false) {
                    continue;
                }

                $table     = $this->_table(true);
                $other_obj = new $include['model'];
                $fields    = $other_obj->_get_columns();
                if (in_array($include['model'], $this->has_one)) {
                    $values = new stdClass();
                    foreach ($fields as $field) {
                        $includedField    = $include['prefix'].$field;
                        $values->{$field} = $this->_data->{$includedField};
                        $other_obj->_data = $other_obj->_data = $values;
                    }
                    $this->_initRelated[$include['model']] = $this->_initRelated[$table] = $other_obj;
                } else if (in_array($include['model'], $this->has_many)) {
                    $fKey = self::_fkid($table);
                    $other_obj->where($fKey, $this->_data->id)->get();
                    $this->_initRelated[$include['model']] = $this->_initRelated[$table] = $other_obj->all;
                }
